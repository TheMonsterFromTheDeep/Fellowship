...
public class Camera {
    ...
    public void drawSprite(Sprite sprite, Renderer renderer) {
        if(sprite.graphic >= 0 && sprite.graphic < graphics.length) {
            renderer.rotate(sprite.getDirection(), (float)graphics[sprite.graphic].getWidth() / 2, (float)graphics[sprite.graphic].getHeight() / 2);
            if(sprite.animated) {                    
                ((AnimGraphic)graphics[sprite.graphic]).render(renderer, x + sprite.getX(), y + sprite.getY(), sprite.animframe);
            }
            else {
    ...
    public void drawToLens(Sprite sprite, Renderer renderer) {
        if(sprite.graphic >= 0 && sprite.graphic < graphics.length) {
            renderer.rotate(sprite.getDirection(), (float)graphics[sprite.graphic].getWidth() / 2, (float)graphics[sprite.graphic].getHeight() / 2);
            if(sprite.animated) {                    
                ((AnimGraphic)graphics[sprite.graphic]).render(renderer, sprite.getX(), sprite.getY(), sprite.animframe);
            }
            else {
    ...
    public void drawToLens(Entity sprite, Renderer renderer) {
        if(sprite.graphic >= 0 && sprite.graphic < graphics.length) {
            graphics[sprite.graphic].render(renderer, sprite.getX(), sprite.getY());//(int)(sprite.getX() - xoff), (int)(sprite.getY() - yoff));
        }
    }
    
    public void drawSprites(World w, Renderer renderer) {
        ...
        for (int i = 0; i < w.getAlignedSize(); i++) {
            if(w.sprites[i].isSprite) {
                Sprite sprite = (Sprite)w.sprites[i];
                if(sprite.graphic >= 0 && sprite.graphic < graphics.length) {
                    renderer.rotate(sprite.getDirection(), (float)graphics[sprite.graphic].getWidth() / 2, (float)graphics[sprite.graphic].getHeight() / 2);
                    if(sprite.animated) {                    
                        ((AnimGraphic)graphics[sprite.graphic]).render(renderer, x + sprite.getX(), y + sprite.getY(), sprite.animframe);
            ...
            if(w.sprites[i].isSprite) {
                ...
                if(sprite.graphic >= 0 && sprite.graphic < graphics.length) {
                    ...
                    if(sprite.animated) {                    
                        ...
                        graphics[sprite.graphic].render(renderer, rx + sprite.getX(), ry + sprite.getY());//(int)(sprite.getX() - xoff), (int)(sprite.getY() - yoff));
                    }
                }
            }
            else {
