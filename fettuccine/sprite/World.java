...
public class World {
    ...
    public World(SpriteSystem system) {
        sprites = new Entity[INIT_SPRITE_COUNT];
    ...
    public void addSprite(Entity s) {   
        if(s.isSprite) {
            ((Sprite)s).parent = this;
            ((Sprite)s).system = this.associatedSystem;
        }
        ...
        sprites = new Entity[tmp.length + GROW_SPRITE_COUNT];
    ...
    public void killAll(Sprite[] exceptions) {
        for(int i = 0; i < sprites.length; i++) {
            boolean kill = true;
            for(Sprite e : exceptions) {
                if(sprites[i] == e) { kill = false; }
            }
            if(kill) {
                sprites[i] = null;
            }
    ...
    public boolean intersects(Sprite s, int type) {
        if(!correctlyAligned) { align(); }
        for(int i = 0; i < alignedSize; i++) {
            if(sprites[i].type == type && sprites[i].isSprite) {
                if(s.collidingWith((Sprite)sprites[i])) {
                    return true;
                }
            }
        }
    ...
    public boolean intersects(Sprite s, Properties p) {
        if(!correctlyAligned) { align(); }
        for(int i = 0; i < alignedSize; i++) {
            if(sprites[i].isSprite) {
                if(((Sprite)sprites[i]).hasProperties(p)) {
                    if(s.collidingWith((Sprite)sprites[i])) {
                        return true;
                    }
                }
    ...
    public boolean intersects(Sprite s, int[] types) {
        if(!correctlyAligned) { align(); }
        for(int i = 0; i < alignedSize; i++) {
            for(int type : types) {
                if(sprites[i].type == type && sprites[i].isSprite) {
    ...
    public Sprite getFirstIntersection(Sprite s) {
        if(!correctlyAligned) { align(); }
        for(int i = 0; i < alignedSize; i++) {
            if(sprites[i].isSprite) {
                if(s.collidingWith((Sprite)sprites[i])) {
    ...
    public Sprite getFirstIntersection(Sprite s, int type) {
        if(!correctlyAligned) { align(); }
        for(int i = 0; i < alignedSize; i++) {
            if(sprites[i].type == type && sprites[i].isSprite) {
                if(s.collidingWith((Sprite)sprites[i])) {
    ...
    private int deNullify() {
        ...
        if(sprites[lastValidIndex] == null) { return -1; }
        if(lastValidIndex == 0) { return 0; }
    ...
    public Entity[] getSprites() { return sprites; }
